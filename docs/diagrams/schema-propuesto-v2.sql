...
-- Versión: 2.0 (Integración con auth.microtv.ar)
-- Fecha: Abril 2026
-- Normalización: BCNF donde razonable, 3NF mínimo
-- =====================================================
--
-- INTEGRACIÓN CON auth.microtv.ar:
-- - auth gestiona: autenticación, contraseñas, sesiones JWT, contexto multi-tenant, roles de identidad
-- - CRM gestiona: perfil operativo de usuarios, roles funcionales del CRM, datos de negocio
-- - crm_users.auth_user_id es FK externa a auth.users.user_id (claim 'sub' del JWT)
...
CREATE TABLE crm_users (
    ...
    -- Referencia externa a auth.microtv.ar (claim 'sub' del JWT)
    auth_user_id VARCHAR(36) NOT NULL UNIQUE,
    
    -- Cache denormalizado de auth (non-authoritative, actualizable desde JWT)
    email VARCHAR(255),
...
auth_user_id referencia a auth.microtv.ar (identidad autoritativa). 
NO contiene credenciales ni gestiona autenticación.';

COMMENT ON COLUMN crm_users.auth_user_id IS 
'user_id proveniente de auth.microtv.ar (claim sub del JWT). 
Identidad autoritativa, clave única.';

COMMENT ON COLUMN crm_users.email IS 
'Cache de auth.users.email. NO autoritativo. 
Actualizar desde JWT claims en cada login.';
...
'Timestamp de última sincronización del cache desde JWT.';
...
'Stock actual por producto (y almacén en futuro)';
...
CREATE TABLE tasks (
    ...
    created_by_crm_user_id UUID NOT NULL REFERENCES crm_users(crm_user_id) ON DELETE SET NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    deleted_at TIMESTAMPTZ,
...
CREATE TABLE subtask_assignments (
    ...
    subtask_id UUID NOT NULL REFERENCES subtasks(subtask_id) ON DELETE CASCADE,
    assigned_crm_user_id UUID NOT NULL REFERENCES crm_users(crm_user_id) ON DELETE CASCADE,
    assigned_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    assigned_by_crm_user_id UUID REFERENCES crm_users(crm_user_id) ON DELETE SET NULL,
...
CREATE TABLE ticket_inventory_requests (
    ...
    requested_by_crm_user_id UUID NOT NULL REFERENCES crm_users(crm_user_id) ON DELETE SET NULL,
...
CREATE TABLE ticket_dispatches (
    ...
    dispatched_by_crm_user_id UUID NOT NULL REFERENCES crm_users(crm_user_id) ON DELETE SET NULL,
...
-- NOTAS FINALES:
-- 1. Este esquema está integrado con auth.microtv.ar
-- 2. auth gestiona: autenticación, contraseñas, sesiones JWT, contexto multi-tenant, roles de identidad
-- 3. CRM gestiona: perfil operativo de usuarios, roles funcionales del CRM, datos de negocio
-- 4. crm_users.auth_user_id es FK externa a auth.users.user_id (claim 'sub' del JWT)
-- 5. crm_roles representa capacidades operativas dentro del CRM, NO roles de autenticación
-- 6. NO hay password_hash, user_sessions, ni lógica de autenticación duplicada
-- 7. El backend del CRM debe validar JWT, extraer claims, y sincronizar crm_users on-demand
